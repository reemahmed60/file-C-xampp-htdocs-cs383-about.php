...
<div style=" font-family: Karma, sans-serif;">
  <div style="text-align: center; text-transform: uppercase;"> <h1>dogs to adopt</h1></div>
...
<div class="row row-padding-box" >
  ...
  while($rows=mysqli_fetch_array($result)){ echo '
    ...
    <div class=" '.$rows['d_loc'].' col-md-3 col-lg-4">
    <div class="card card-body h-100">

    <form action="adoptionform.php" method= "get">
    <p><img src="uploaded/'.$rows['d_img'].'" width= "100%" height="100%" "/></p>
     <p>Location: '.$rows['d_loc'].'</p>
     <p>Gender: '.$rows['d_gender'].'</p>
     <p>Age: '.$rows['d_age'].'</p>
     <p>Name: '.$rows['d_name'].'</p>
     <p>Info: '.$rows['d_info'].'</p>
     <p><button class="btn" type="submit" >Ask for Adoption</button></p>
    <input type="hidden" name="pet" value="'.$rows['d_name'].'"></input>
    </form>
    </div>
    </div>';} ?>
